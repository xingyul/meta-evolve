...
def interp_eval(interp_vector, agent, args, num_eval, seed=0, log_string=print, **env_kwargs):
    env_kwargs = {'id': args.generalized_env, 'interp_param_vector': interp_vector, 'dense_reward': False}
    env = gym.make(**env_kwargs)
    success = 0

    input_dict = dict(num_traj=num_eval, env=env_kwargs, policy=agent.policy, \
            horizon=args.horizon, base_seed=seed, num_cpu=num_eval, eval_mode=True)
    paths = trajectory_sampler.sample_paths(**input_dict)
    success_rate_eval = env.env.evaluate_success(paths) / 100.
    log_string('num eval: {}'.format(num_eval))
...
def interp_forward(target_interp_vector, agent, args, \
