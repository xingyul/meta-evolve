...
parser.add_argument('--round_id', type=int, default=0, help='activation dim expansion')
...
for start_robot_name, end_robot_name in tree.tree:
    log_dir = os.path.join(args.log_dir, \
            'log_{}_{}'.format(start_robot_name, end_robot_name))
    ...
    while True:
        ...
        if np.linalg.norm(interp_param_vector - goal_interp_param) < args.interp_progression:
            ...
            success_rate_eval, eval_rewards = interp_utils.interp_eval( \
                    interp_param_vector + forward_vec, agent, args, \
                    num_eval=num_eval, seed=agent.seed+2**31+iteration, log_string=log_string)
        ...
        if success_rate_eval < eval_success_rate_thres:
            ...
            agent_return, mean_eval_reward = interp_utils.interp_forward( \
                    ...
                    sample_direction=sample_vec, eval_freq=5, num_eval=num_eval, \
