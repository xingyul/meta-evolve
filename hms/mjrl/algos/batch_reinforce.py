...
class BatchREINFORCE:
    ...
    def train_step(self, N,
        ...
        while True:
            ...
            if max_reward_returns > min_train_max_reward:
                break
        ...
        if self.save_logs:
            ...
            self.logger.log_kv('time_train', timer.time()-ts)
    ...
    def train_from_paths(self, paths):
        ...
        if self.save_logs:
            ...
            try:
                success_rate = self.env.env.env.evaluate_success(paths, self.logger)
                self.logger.log_kv('success_rate', success_rate)
            except:
                try:
                    success_rate = self.env[0].env.env.evaluate_success(paths)
                    self.logger.log_kv('success_rate', success_rate)
                except:
                    pass
    ...
    def process_paths(self, paths):
        ...
        try:
            ...
            try:
                success_rate = self.env[0].env.evaluate_success(paths)
            except:
                success_rate = self.env.evaluate_success(paths)
        base_stats = {'success_rate': success_rate,
