...
def policy_expansion( \
    ...
    if num_obs_expansion > 0:
        ...
        new_in_shift = np.concatenate([policy_expanded.model.in_shift.numpy(), \
                np.zeros([num_obs_expansion], dtype='float32')])
        new_in_shift = torch.from_numpy(new_in_shift)
        new_in_scale = np.concatenate([policy_expanded.model.in_scale.numpy(), \
                np.ones([num_obs_expansion], dtype='float32')])
        ...
        new_in_shift = np.concatenate([policy_expanded.old_model.in_shift.numpy(), \
                np.zeros([num_obs_expansion], dtype='float32')])
        new_in_shift = torch.from_numpy(new_in_shift)
        new_in_scale = np.concatenate([policy_expanded.old_model.in_scale.numpy(), \
                np.ones([num_obs_expansion], dtype='float32')])
        new_in_scale = torch.from_numpy(new_in_scale)
    ...
    if num_act_expansion > 0:
        ...
        new_out_shift = np.concatenate([policy_expanded.model.out_shift.numpy(), \
                np.zeros([num_act_expansion], dtype='float32')])
        new_out_shift = torch.from_numpy(new_out_shift)
        new_out_scale = np.concatenate([policy_expanded.model.out_scale.numpy(), \
                np.ones([num_act_expansion], dtype='float32')])
        ...
        new_out_shift = np.concatenate([policy_expanded.old_model.out_shift.numpy(), \
                np.zeros([num_act_expansion], dtype='float32')])
        new_out_shift = torch.from_numpy(new_out_shift)
        new_out_scale = np.concatenate([policy_expanded.old_model.out_scale.numpy(), \
                np.ones([num_act_expansion], dtype='float32')])
        new_out_scale = torch.from_numpy(new_out_scale)
...
def baseline_expansion( \
    ...
    if num_obs_expansion > 0:
        ...
        new_input_module = nn.Linear(original_in_dim + num_obs_expansion, \
                input_module.out_features)
        new_input_module.weight.data *= 0.
        new_input_module.weight.data[:, :(original_in_dim-4)] = \
                input_module.weight.data[:, :(original_in_dim-4)]
        ##### last four dimensions are fixed
        new_input_module.weight.data[:, -4:] = \
                input_module.weight.data[:, -4:]
        new_input_module.bias.data = input_module.bias.data
