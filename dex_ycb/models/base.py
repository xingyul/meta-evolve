...
from utils.mjcf_utils import find_elements, sort_elements,\
...
class MujocoXML(object):
    ...
    def __init__(self, fname=None, xml_string=None, folder=None):
        if fname is None:
        ...
        self.default_classes = self._get_default_classes(self.default)
        worldbody = find_elements(self.root, tags='worldbody')
        self._replace_defaults_inline(default_dic=self.default_classes, \
                root=worldbody)
    ...
    def _get_default_classes(default):
        ...
        children = [elem for elem in default]
        for elem in children:
            if elem.tag != 'default':
                default_dic[elem.tag] = {att: elem.attrib[att] for att in elem.attrib}
                default.remove(elem)
        return default_dic
    ...
    def _replace_defaults_inline(self, default_dic, root=None):
        ...
        ##### all default tag to attrib mapping
        default_classes = [d for d in self.root.iter() if d.tag == 'default']
        default_classes = [d for d in default_classes if 'class' in d.attrib]
        default_classes = {d.attrib['class']: [t for t in d] for d in default_classes if 'main' not in d.attrib['class']}

        worldbody = find_elements(self.root, tags='worldbody')
        actuators = find_elements(self.root, tags='actuator')
        # Check this current element if it contains any class elements
        for tag in default_dic:
            tag_attrs = default_dic[tag]
            if tag == 'tendon':
                tendon_root = [elem for elem in self.root if elem.tag == tag][0]
                elems = [elem for elem in tendon_root]
            else:
                elems = [elem for elem in worldbody.iter() if elem.tag == tag] +\
                        [elem for elem in actuators.iter() if elem.tag == tag]
            for elem in elems:
                if 'class' in elem.attrib:
                    default_tags = default_classes[elem.attrib['class']]
                    default_class_attrib = [d for d in default_tags if d.tag == elem.tag][0].attrib
                    main_class_attrib = default_dic[elem.tag]
                    for key, value in main_class_attrib.items():
                        if key in default_class_attrib:
                            elem.attrib[key] = default_class_attrib[key]
                        else:
                            elem.attrib[key] = main_class_attrib[key]
                else:
