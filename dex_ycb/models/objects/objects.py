...
from models.base import MujocoXML, MujocoModel
from utils.mjcf_utils import string_to_array, array_to_string, CustomMaterial, OBJECT_COLLISION_COLOR,\
...
class MujocoXMLObject(MujocoObject, MujocoXML):
    ...
    def _get_object_subtree(self):
        ...
        if obj is None:
            obj = copy.deepcopy(self.worldbody.find("./body[@name='object']"))
        ...
        for i, (parent, element) in enumerate(geom_pairs):
            ...
            if not _should_keep(element):
                ...
                g_name = g_name if g_name is not None else "g{}".format(i)
