    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def gripper_to_cubeA(obs_cache):
                return obs_cache["cubeA_pos"] - obs_cache["{pf}eef_pos"] if \
                    "cubeA_pos" in obs_cache and "{pf}eef_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def gripper_to_cubeB(obs_cache):
                return obs_cache["cubeB_pos"] - obs_cache["{pf}eef_pos"] if \
                    "cubeB_pos" in obs_cache and "{pf}eef_pos" in obs_cache else np.zeros(3)
