...
from utils.placement_samplers import SequentialCompositeSampler, UniformRandomSampler
    ...
    def _load_model(self):
        ...
        for i, (nut_cls, nut_name) in enumerate(zip(
            ...
            if isinstance(self.placement_initializer, SequentialCompositeSampler):
                ...
                self.placement_initializer.add_objects_to_sampler(sampler_name="{nut_name}Sampler", mujoco_objects=nut)
            else:
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def world_pose_in_gripper(obs_cache):
                return T.pose_inv(T.pose2mat((obs_cache["{pf}eef_pos"], obs_cache["{pf}eef_quat"]))) if\
                    "{pf}eef_pos" in obs_cache and "{pf}eef_quat" in obs_cache else np.eye(4)
    ...
    def _create_nut_sensors(self, nut_name, modality="object"):
        ...
        def nut_to_eef_pos(obs_cache):
            ...
            if any([name not in obs_cache for name in
                    ["{nut_name}_pos", "{nut_name}_quat", "world_pose_in_gripper"]]):
                return np.zeros(3)
            obj_pose = T.pose2mat((obs_cache["{nut_name}_pos"], obs_cache["{nut_name}_quat"]))
            rel_pose = T.pose_in_A_to_pose_in_B(obj_pose, obs_cache["world_pose_in_gripper"])
            rel_pos, rel_quat = T.mat2pose(rel_pose)
            obs_cache["{nut_name}_to_{pf}eef_quat"] = rel_quat
            return rel_pos

        @sensor(modality=modality)
        def nut_to_eef_quat(obs_cache):
            return obs_cache["{nut_name}_to_{pf}eef_quat"] if \
                "{nut_name}_to_{pf}eef_quat" in obs_cache else np.zeros(4)

        sensors = [nut_pos, nut_quat, nut_to_eef_pos, nut_to_eef_quat]
        names = ["{nut_name}_pos", "{nut_name}_quat", "{nut_name}_to_{pf}eef_pos", "{nut_name}_to_{pf}eef_quat"]
