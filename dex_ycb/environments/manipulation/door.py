...
from models.objects import DoorObject
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def door_to_eef_pos(obs_cache):
                return obs_cache["door_pos"] - obs_cache["{pf}eef_pos"] if\
                    "door_pos" in obs_cache and "{pf}eef_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def handle_to_eef_pos(obs_cache):
                return obs_cache["handle_pos"] - obs_cache["{pf}eef_pos"] if\
                    "handle_pos" in obs_cache and "{pf}eef_pos" in obs_cache else np.zeros(3)
