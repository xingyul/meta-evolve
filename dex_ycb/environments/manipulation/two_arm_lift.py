...
class TwoArmLift(TwoArmEnv):
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def gripper0_to_handle0(obs_cache):
                return obs_cache["handle0_xpos"] - obs_cache["{}eef_pos".format(pf0)] if \
                    "handle0_xpos" in obs_cache and "{}eef_pos".format(pf0) in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def gripper1_to_handle1(obs_cache):
                return obs_cache["handle1_xpos"] - obs_cache["{}eef_pos".format(pf1)] if \
                    "handle1_xpos" in obs_cache and "{}eef_pos".format(pf1) in obs_cache else np.zeros(3)
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            handles = [self.pot.important_sites["handle{}".format(i)] for i in range(2)]
