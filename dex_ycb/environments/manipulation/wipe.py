...
from models.arenas import WipeArena
...
class Wipe(SingleArmEnv):
            ...
            if self.use_condensed_obj_obs:
                ...
                if self.use_robot_obs:
                    ...
                    def gripper_to_wipe_centroid(obs_cache):
                        return obs_cache["wipe_centroid"] - obs_cache["{pf}eef_pos"] if \
                            "wipe_centroid" in obs_cache and "{pf}eef_pos" in obs_cache else np.zeros(3)
    ...
    def _create_marker_sensors(self, i, marker, modality="object"):
        ...
        names = ["marker{i}_pos", "marker{i}_wiped"]

        if self.use_robot_obs:
            # also use ego-centric obs
            @sensor(modality=modality)
            def gripper_to_marker(obs_cache):
                return obs_cache["marker{i}_pos"] - obs_cache["{pf}eef_pos"] if \
                    "marker{i}_pos" in obs_cache and "{pf}eef_pos" in obs_cache else np.zeros(3)

            sensors.append(gripper_to_marker)
            names.append("gripper_to_marker{i}")
