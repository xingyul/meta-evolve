...
class PickPlace(SingleArmEnv):
    ...
    def _get_placement_initializer(self):
        ...
        for vis_obj in self.visual_objects:
            ...
            self.placement_initializer.append_sampler(
                sampler=UniformRandomSampler(
                    name="{vis_obj.name}ObjectSampler",
    ...
    def _create_obj_sensors(self, obj_name, modality="object"):
        ...
        def obj_to_eef_pos(obs_cache):
            ...
            if any([name not in obs_cache for name in
                    ["{obj_name}_pos", "{obj_name}_quat", "world_pose_in_gripper"]]):
                return np.zeros(3)
            obj_pose = T.pose2mat((obs_cache["{obj_name}_pos"], obs_cache["{obj_name}_quat"]))
            rel_pose = T.pose_in_A_to_pose_in_B(obj_pose, obs_cache["world_pose_in_gripper"])
            rel_pos, rel_quat = T.mat2pose(rel_pose)
            obs_cache["{obj_name}_to_{pf}eef_quat"] = rel_quat
            return rel_pos

        @sensor(modality=modality)
        def obj_to_eef_quat(obs_cache):
            return obs_cache["{obj_name}_to_{pf}eef_quat"] if \
                "{obj_name}_to_{pf}eef_quat" in obs_cache else np.zeros(4)

        sensors = [obj_pos, obj_quat, obj_to_eef_pos, obj_to_eef_quat]
        names = ["{obj_name}_pos", "{obj_name}_quat", "{obj_name}_to_{pf}eef_pos", "{obj_name}_to_{pf}eef_quat"]
