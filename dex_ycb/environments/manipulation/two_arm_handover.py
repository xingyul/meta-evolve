...
from models.objects import HammerObject
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def gripper0_to_handle(obs_cache):
                return obs_cache["handle_xpos"] - obs_cache["{pf0}eef_pos"] if \
                    "handle_xpos" in obs_cache and "{pf0}eef_pos" in obs_cache else np.zeros(3)

            @sensor(modality=modality)
            def gripper1_to_handle(obs_cache):
                return obs_cache["handle_xpos"] - obs_cache["{pf1}eef_pos"] if \
                    "handle_xpos" in obs_cache and "{pf1}eef_pos" in obs_cache else np.zeros(3)
